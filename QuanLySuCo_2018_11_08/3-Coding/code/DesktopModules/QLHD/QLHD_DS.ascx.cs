...
namespace QLHD
{
    public partial class QLHD_DS : DotNetNuke.Entities.Modules.UserModuleBase
    {
        #region Khai báo, định nghĩa đối tượng
        private readonly ILog log = LoggerSource.Instance.GetLogger(typeof(QLHD_DS).FullName);
        ...
        protected void dgDanhSach_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                for (int i = 1; i <= 11; i++)
                {
                    e.Item.Cells[i].Attributes.Add("onmouseover", "this.style.cursor='hand'");
                    e.Item.Cells[i].Attributes.Add("onclick", "javascript:__doPostBack" +
                    "('" + e.Item.UniqueID + "$hplChiTiet' , '')");
                }
            }
