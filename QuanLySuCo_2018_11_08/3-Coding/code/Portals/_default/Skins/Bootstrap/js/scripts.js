$(document).ready(function () {
    //Kiem tra co submenu khong
    if ($(".fixed-menu .dropdown-submenu.level2").length) {
        var newPos = $(".fixed-menu .dropdown-submenu.level2").offset().left - $(this).width();
        $(".fixed-menu .dropdown-submenu.level2").find('.dropdown-menu.level2').offset({ "left": newPos });

        $(".fixed-menu .dropdown-submenu.level2").mouseover(function () {
            var submenuPos = $(this).offset().left + $(this).width() + $(this).find('.dropdown-menu.level2').width();
            var windowPos = $(window).width();
            var oldPos = $(this).offset().left + $(this).width();
            var newPos = ($(this).offset().left - $(this).find('.dropdown-menu.level2').width()) + 20;

            if (submenuPos > windowPos) {
                $(this).find('.dropdown-menu.level2').offset({ "left": newPos });
            } else {
                $(this).find('.dropdown-menu.level2').offset({ "left": oldPos });
            }
        });
    }
    if ($(".fixed-menu .dropdown-submenu.level3").length > 0) {
        var newPos3 = $(".fixed-menu .dropdown-submenu.level3").offset().left - $(this).width();
        $(".fixed-menu .dropdown-submenu.level3").find('.dropdown-menu.level3').offset({ "left": newPos3 });

        $(".fixed-menu .dropdown-submenu.level3").mouseover(function () {
            //alert($(this).offset().left);
            var submenuPos3 = $(this).offset().left + $(this).width() + $(this).find('.dropdown-menu.level3').width();
            var windowPos3 = $(window).width();
            var oldPos3 = $(this).offset().left + $(this).width();
            var newPos3 = ($(this).offset().left - $(this).find('.dropdown-menu.level3').width()) + 20;

            if (submenuPos3 > windowPos3) {
                $(this).find('.dropdown-menu.level3').offset({ "left": newPos3 });
            } else {
                $(this).find('.dropdown-menu.level3').offset({ "left": oldPos3 });
            }
        });
    }
    ...
    $(".fixed-menu .dropdown").mouseover(function () {
        var submenuPosa = $(this).offset().left + 200;
        var windowPosa = $(window).width();
        var oldPosa = $(this).offset().right;
        var newPosa = $(this).offset().left - $(this).width();
        if (submenuPosa > windowPosa) {
            $(this).find('.dropdown-menu.level1').css({ "right": "0", "left": "inherit" });
        } else {
    ...
    $(window).scroll(function () {
        ...
        //        if (scroll > 100) {
        //            $(".js-clingify-wrapper").addClass("js-clingify-locked"); //removeClass("clearHeader")
        //        }
        //        else {
        //            $(".js-clingify-wrapper").removeClass("js-clingify-locked");
        //        }
    });
